...
export async function getTvshowEpisodes(tvshowId: number, seasonNumber: number) {
  const res = await fetch(`https://api.themoviedb.org/3/tv/${tvshowId}/season/${seasonNumber}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en-US`);
  if (!res.ok) {
    throw new Error("Failed to fetch TV show episodes");
  }
  return res.json();
}
