...
import type { Movie } from "@/lib/fetchMovies";

export default async function HomePage() {
  const movies = await getTrendingMovies();
  const tvShows = await getTrendingTVShows();
  const movieArray: Movie[] = Array.isArray(movies) ? movies : [];
  const tvShowArray: Movie[] = Array.isArray(tvShows) ? tvShows : [];
  ...
  return (
    <main className="md:p-4 p-1 bg-gray-900 text-white">
      <MovieSlider movies={movieArray.slice(0, 10)} />
