...
// Import or define the missing functions and constant
import { getTvshowDetails, getEpisodesBySeason, getTvshowCast } from "@/utils/api"; // Adjust the path as needed
const API_KEY = process.env.NEXT_PUBLIC_TMDB_API_KEY; // Ensure this is set in your environment variables
...
export default async function TVpage({
  ...
  const tvshowDetail = await (
    await fetch(
  // Removed unused variables 'episodesBySeason' and 'cast'
  await getEpisodesBySeason(tvshow.id, tvshowDetail.seasons);
  await getTvshowCast(tvshow.id);
    tvshow.id,
        ...
        <div className="mx-4 text-ellipsis">
          ...
          <div>
            <h1>{tvshow?.overview}</h1>
                ? tvshow.genres.map((genre: { name: string }) => genre.name).join(", ")
