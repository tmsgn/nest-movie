...
type Tvshow = { id: number; name: string; poster_path: string; overview: string; first_air_date: string; vote_average: number };
type Episode = { season_number: number; episode_number: number; name: string; overview: string; still_path: string };
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-4 mt-2">
            ...
            <span className="bg-yellow-500 text-gray-700 font-medium px-1 rounded text-xs shadow">HD</span>
          </div>
          <p className="mt-4">{tvshow.overview}</p>
          <div className="flex flex-wrap gap-2 mt-4">
            {tvshowDetail.genres.map((g: any) => (
              <span key={g.id} className="text-sm font-medium text-gray-700 bg-yellow-400 px-2 py-1 rounded">{g.name}</span>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
          {cast.slice(0, 6).map((member, index) => (
            <div key={index} className="flex flex-col items-center">
              ...
              <h2 className="text-sm font-semibold mt-2 line-clamp-1">{member.name}</h2>
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {currentEpisodes.map((episode) => (
            <div key={episode.episode_number} className="bg-gray-900 rounded-lg overflow-hidden shadow w-32 sm:w-36 md:w-40 lg:w-44 xl:w-48">
              <img
              ...
              />
              <div className="p-1">
                <div className="font-semibold text-white text-sm truncate" title={episode.name}>{episode.name}</div>
        ...
        <div className="flex justify-center gap-4 mt-4">
          <button
            ...
            className={`px-4 py-2 rounded cursor-pointer ${currentPage === 1 ? 'bg-gray-600 cursor-not-allowed' : 'bg-yellow-400 text-gray-800'}`}
          >
            Prev
          </button>
          <button
            ...
            className={`px-4 py-2 rounded ${currentPage === totalPages || totalPages === 0 ? 'bg-gray-600 cursor-not-allowed' : 'bg-yellow-400 text-gray-800'}`}
          >
            Next
          </button>
        </div>
