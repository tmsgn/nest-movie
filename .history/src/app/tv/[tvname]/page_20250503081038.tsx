...
export default async function TVpage({
  ...
  return (
    <div>
      <div className="w-screen flex md:flex-row ">
        ...
        <div className="w-full md:px-7  md:mt-0 mt-5">
          {trailer ? (
            <div className="">
      ...
      <div className="">
        <h1 className="text-2xl font-bold text-yellow-400 m-3">Casts</h1>
        <div className="grid lg:grid-cols-8 md:grid-cols-5 sm:grid-cols-4 grid-cols-3 md:gap-3 gap-1">
          {cast.map((member, index) => (
            <div key={index} className="flex flex-col items-center">
              ...
              <h2 className="text-xs font-semibold line-clamp-1">{member.name}</h2>
              <p className="text-xs text-gray-400 line-clamp-1">{member.character}</p>
            </div>
          ))}
        </div>
        <div>
          <h1 className="text-2xl font-bold text-yellow-400 m-3">Episodes</h1>
          {Object.keys(episodesBySeason).map((seasonNumber) => (
            <div key={seasonNumber} className="mb-4">
              <h2 className="text-xl font-semibold">Season {seasonNumber}</h2>
              <ul className="list-disc pl-5">
                {episodesBySeason[parseInt(seasonNumber)].map((episode) => (
                  <li key={episode.episode_number} className="mb-2">
