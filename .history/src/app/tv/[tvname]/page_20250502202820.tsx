...
// Ensured Tvshow type is used in the code
type Tvshow = {
  id: number;
  title: string;
  poster_path: string;
...
export default async function TVpage({
  ...
  // Removed unused episodesBySeason variable
      ...
      <div className="w-screen flex sm:flex-row flex-col">
        ...
        <div className="mx-4 text-ellipsis">
          ...
          <div className="max-w-2xl">
            ...
            {/* Cast */}
            {tvshowCast && tvshowCast.length > 0 && (
            {cast && cast.length > 0 && (
              <div className="mt-4">
                <h2 className="text-lg font-bold">Cast:</h2>
                <div className="flex flex-wrap">
                  {cast.map((member: { name: string; character: string }) => (
                    <div
                      key={member.name}
                      className="bg-gray-800 text-gray-200 text-sm font-medium px-2 py-1 rounded mr-2 mb-2"
                    >
