...
export default async function TVpage({
  ...
  // Check the params to see if both ID and title are being extracted
  const [idStr, titleSlug] = params.tvname.split("-");

  const id = parseInt(idStr); // Parse the ID as an integer
  const title = decodeURIComponent(titleSlug.replace(/-/g, " ")); // Decode the title and replace hyphens with spaces

  const tvshow = await getTvshowDetails(title);

  if (!tvshow || tvshow.id !== id) {
