...
const getTvshowDetails = async (title: string): Promise<Tvshow | null> => {
  try {
    ...
    console.log("API Response for TV show search:", data); // Debugging log
...
export default async function TVpage({
  ...
  try {
    ...
    const genres = tvshowDetail.genres?.map((g: { id: number; name: string }) => ({
      id: g.id,
      name: g.name,
    })) || [];
    const seasons = tvshowDetail.seasons || [];
    const episodesBySeason = await getEpisodesBySeason(parseInt(tvshowId), seasons);
