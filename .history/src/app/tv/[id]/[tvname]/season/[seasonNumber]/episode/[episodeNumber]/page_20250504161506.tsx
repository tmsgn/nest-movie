    ...
    async function fetchTvshow() {
      if (id) {
        ...
        if (data && data.seasons) {
          const keys = data.seasons.map((s: any) => s.season_number).filter((num: number) => num > 0);
          setSeasonKeys(keys);
        }
      }
    }
              ...
              <div className="ml-4 flex flex-col justify-start space-y-2">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {Array.from({ length: 3 }).map((_, index) => (
                    <div key={index} className="h-6 bg-yellow-400 rounded px-4 py-2 animate-pulse"></div>
                  ))}
                </div>
              </div>
            </div>
            <div>
              ...
              <div className="mt-3 space-y-2">
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={index} className="h-4 bg-gray-600 rounded w-full animate-pulse"></div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 ml-4">
          <div className="flex items-center gap-2 mb-4 relative">
            <button
              ...
              className="bg-gray-800 text-white max-h-40 overflow-y-scroll scrollbar px-4 py-2 rounded-lg text-lg font-semibold flex items-center gap-2 cursor-pointer"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
            {currentEpisodes.map((episode) => (
              <Link
                ...
                className="bg-gray-900 rounded-lg cursor-pointer transition-transform transform hover:scale-105 overflow-hidden shadow w-32 sm:w-36 md:w-40 lg:w-44 xl:w-48"
              >
                <img
                  src={
                    episode.still_path
