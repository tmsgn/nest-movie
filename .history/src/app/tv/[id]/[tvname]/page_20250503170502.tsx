...
type Tvshow = {
  ...
  genres: string[]; // final genre names array
};
...
function slugify(title: string): string {
  return encodeURIComponent(title.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]+/g, ""));
}

async function getTvshowDetails(title: string): Promise<Tvshow | null> {
  try {
    const res = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=${API_KEY}&query=${title}`);
...
async function getTvshowCast(tvshowId: number): Promise<CastMember[]> {
  try {
    const res = await fetch(`https://api.themoviedb.org/3/tv/${tvshowId}/credits?api_key=${API_KEY}`);
...
async function getEpisodesBySeason(tvshowId: number, seasons: { id: number; season_number: number }[]): Promise<{ [seasonNumber: number]: Episode[] }> {
  const episodesBySeason: { [seasonNumber: number]: Episode[] } = {};

  await Promise.all(seasons.map(async (season) => {
    if (season.season_number === 0) return;
    try {
      const res = await fetch(`https://api.themoviedb.org/3/tv/${tvshowId}/season/${season.season_number}?api_key=${API_KEY}`);
      if (!res.ok) throw new Error(`Failed to fetch season ${season.season_number}`);
      const data = await res.json();
      episodesBySeason[season.season_number] = data.episodes || [];
    } catch (err) {
...
export default async function TVpage({ params }: { params: { tvname: string } }) {
  ...
  const genres = Array.isArray(tvshowDetail.genres) ? tvshowDetail.genres.map((g: { name: string }) => g.name) : [];
