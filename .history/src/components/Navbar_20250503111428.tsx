...
type Movie = {
  ...
  release_date: string;
  media_type?: string; // optional if missing
};
...
export default function NavBar() {
        ...
        .then((data) => {
          if (data && data.length > 0) {
            ...
            setResults(
              result.map((item) => ({
                ...item.item,
                media_type: item.item.media_type ?? "movie", // fallback to movie if missing
              }))
            );
          } else {
            setResults([]);
  ...
  const handleSearchSubmit = (event: React.FormEvent) => {
    ...
    if (normalizedQuery) {
      router.push(`/search/${encodeURIComponent(normalizedQuery.replace(/\s+/g, "-"))}`);
    }
    setQuery("");
  };
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold line-clamp-1 text-xs md:text-base">{movie.title}</span>
                        <span className="text-xs md:text-sm text-gray-400">{movie.release_date || "Unknown"}</span>
