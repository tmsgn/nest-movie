...
export default function NavBar() {
      ...
      searchMovies(query)
        .then((data) => {
          ...
          const filtered = fuse.search(normalized).map(item => ({
            ...
            release_date: item.item.release_date ?? item.item.first_air_date ?? "",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (normalizedQuery) router.push(`/search/${encodeURIComponent(normalizedQuery)}`);
    setQuery("");
  };
    ...
    <nav className="bg-gray-900 text-white px-4 py-3 shadow-md w-full">
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <ul className="flex gap-6">
          ...
          <li><Link href="/favorites" className="hover:text-yellow-300">Favorites</Link></li>
            ...
            <div className="absolute left-0 right-0 top-12 max-h-60 overflow-y-auto bg-gray-800 text-white rounded-md shadow-lg z-20">
              {loading ? (
                ...
                results.filter(movie => movie.poster_path).map(movie => (
                  <Link
                    key={movie.id}
                    href={`/${movie.media_type}/${encodeURIComponent(movie.title.toLowerCase().replace(/\s+/g, "-"))}`}
                    onClick={() => setQuery("")}
                    className="block px-4 py-2 hover:bg-yellow-300 rounded transition"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <span className="font-bold text-xs md:text-base">{movie.title}</span>
                        <div className="text-xs text-gray-400">{movie.release_date || "Unknown Release Date"}</div>
                        <div className="text-xs italic text-gray-500">{movie.media_type === "tv" ? "TV Show" : "Movie"}</div>
                      </div>
                    </div>
                  </Link>
                ))
              ) : (
                <div className="px-4 py-2 text-center text-gray-400">No results found.</div>
              )}
            </div>
          )}
        </form>
