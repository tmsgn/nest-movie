...
const handlePlayClick = (movie: MovieSliderItem) => {
    if (movie.media_type === "movie") {
        router.push(`/movie/${movie.id}/${encodeURIComponent(movie.title ?? movie.name ?? "untitled")}`);
    } else if (movie.media_type === "tv") {
        router.push(`/tv/${movie.id}/season/1/episode/1`); // Defaulting to season 1, episode 1
    }
};
