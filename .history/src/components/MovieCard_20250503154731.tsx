...
interface Movie {
  ...
  vote_average: number;
  genre_ids?: number[];
  // Keeping genres type flexible based on potential data sources (list vs detail)
...
export default function MovieCard({ movie }: { movie: Movie }) {
  ...
  // Used for the movie route (/movie/[moviename]) and the tv name part (/tv/[id]/[tvname])
  ...
  // For 'movie', the route is /movie/[moviename] (as requested)
  const href = movie.media_type === "tv" ?
               `/tv/${movie.id}/${slug}` :
               `/movie/${slug}`; // Link to /movie/[moviename] using the title slug

    // Get the first genre name, handling different possible genre formats
    const firstGenreName = (movie.genres && movie.genres.length > 0) ?
                         ...
                         ? `Genre ID: ${movie.genre_ids[0]}` // If only genre IDs are available, display ID or perform lookup
                         : null;
