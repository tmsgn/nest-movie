...
// Updated interface to match potential data from list endpoints
interface Movie {
  id: number;
  title?: string;
  name?: string;
...
export default function MovieCard({ movie }: { movie: Movie }) {
  const [isClicked, setIsClicked] = useState(false); // For handling click state
  ...
  const slug = itemTitle
    ? encodeURIComponent(itemTitle.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]+/g, ""))
    : "detail";
  ...
  const handleClick = () => {
    setIsClicked(true);
    setTimeout(() => {
      // Redirect to the movie or TV show page after a short delay
      window.location.href = href;
    }, 300); // delay before navigating to the next page
  };
  ...
  return (
    <div className="group block h-full">
      <Link href={href} passHref>
        <div
          className={`cursor-pointer rounded-lg p-1 group transition-transform transform hover:scale-105 relative h-full flex flex-col ${isClicked ? "opacity-50" : ""}`}
          onClick={handleClick}
        >
          ...
          <div className="mt-2 flex-grow flex flex-col justify-between">
            {releaseYear && <h1 className="text-xs md:text-sm font-medium text-gray-400 ">{releaseYear}</h1>}
            <div>
              <h1 className="font-bold line-clamp-1 text-xs md:text-base">{itemTitle}</h1>
              <div className="flex justify-between items-center flex-wrap gap-1">
                ...
                {movie.vote_average > 0 && <h1 className="text-xs flex-shrink-0">{movie.vote_average.toFixed(1)}⭐</h1>}
              </div>
            </div>
          </div>
        </div>
